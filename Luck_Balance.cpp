...
int luckBalance(int k, vector<vector<int>> contests) {
    int count = 0;
    vector<int> value_of_1s(101,0);
    int vec_point = 0;
    int cont_size = contests.size();
    for (int i = 0; i < cont_size; ++i) {
        if(contests[i][1] == 0){
            count += contests[i][0];
        }
        else{
            value_of_1s[vec_point++] = contests[i][0];
        }
    }
    vector<int> tmp(vec_point,0);
    for (int i = 0; i < vec_point; ++i) {
        tmp[i] = value_of_1s[i];
    }
